...
def test_logout_redirects_home(client, django_user_model):
    # log a user in first
    u = django_user_model.objects.create_user(username="u1", email="u1@example.com", password="Passw0rd1!")
    client.post(reverse(f"{APP}:login"), {"username": "u1", "password": "Passw0rd1!"}, follow=True)
...
def test_profile_edit_get_and_post_invalid(client, django_user_model):
    u = django_user_model.objects.create_user(username="ed", email="ed@example.com", password="Passw0rd1!")
    # login
    client.post(reverse(f"{APP}:login"), {"username": "ed", "password": "Passw0rd1!"}, follow=True)
