...
def test_details_view_success(
    logged_in_attendee_client, details_url, details_ticket, test_event
):
...
def profile1_list(user1_list):
    ...
    profile, _ = UserProfile.objects.get_or_create(
        user=user1_list, defaults={"role": "attendee"}
    )
    return profile
...
def profile2_list(user2_list):
    ...
    profile, _ = UserProfile.objects.get_or_create(
        user=user2_list, defaults={"role": "attendee"}
    )
    return profile
...
def list_ticket_info1(list_test_event):
    ...
    return TicketInfo.objects.create(
        event=list_test_event, category="General Admission", price=10
    )
...
def ticket1_user1(profile1_list, list_ticket_info1):
    ...
    return Ticket.objects.create(
        attendee=profile1_list, ticketInfo=list_ticket_info1, full_name="User One GA"
    )
...
def ticket2_user1(profile1_list, list_ticket_info2):
    ...
    return Ticket.objects.create(
        attendee=profile1_list, ticketInfo=list_ticket_info2, full_name="User One VIP"
    )
...
def ticket1_user2(profile2_list, list_ticket_info1):
    ...
    return Ticket.objects.create(
        attendee=profile2_list, ticketInfo=list_ticket_info1, full_name="User Two GA"
    )
...
def ticket_guest(list_ticket_info1):
    ...
    return Ticket.objects.create(
        attendee=None, ticketInfo=list_ticket_info1, full_name="Guest"
    )
...
def test_ticket_list_attendee_role(
    ...
    list_url,
    ticket1_user1,
    ticket2_user1,  # Include ticket fixtures to ensure creation
):
    """Test list view when user has 'attendee' role in session."""
    # Log in user1 AS attendee using the custom login view
    login_data = {"username": user1_list.username, "password": "Passw0rd1!"}
